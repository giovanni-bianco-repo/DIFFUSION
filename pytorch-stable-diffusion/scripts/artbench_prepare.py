...
# Create directory
os.makedirs("cubist_dataset", exist_ok=True)
...
source_dir = Path("artbench_data/train/cubism_2")
dest_dir = Path("cubist_dataset")

count = 0
for img_file in source_dir.glob("*.jpg"):
    if count < 50:  # Get first 50 images
        img = Image.open(img_file)
        img.save(dest_dir / f"cubist_{count:03d}.jpg")
        count += 1
